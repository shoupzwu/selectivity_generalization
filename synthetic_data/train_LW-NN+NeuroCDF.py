import os
import copy
import numpy as np
import torch
from model import MLP as NeuCDF
import random
import json
from utilites import *
from NeuroCDF_helpers import *

random.seed(0)
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"

def extract_sublist(original_list, indices):
	return [original_list[i] for i in indices]

num_dim = 10
num_samples = 50000

epoch = 1000
bs = 1000
candi_size = 50000
test_size = 1000
feature_dim = 256
q_file = 'queries/queries_50000.csv'
model = NeuCDF(num_dim, feature_dim)


optimizer = torch.optim.Adam(model.parameters(), lr=5e-4)

training_queries, training_intervals, training_sels, test_queries, test_intervals, \
test_sels, ood_test_queries, ood_test_intervals, ood_test_sels = get_queries(q_file)

print("training size: {}".format(len(training_intervals)))
print("test size: {}".format(len(test_intervals)))
print("ood test size: {}".format(len(ood_test_sels)))

num_batches = int(len(training_queries) / bs) + 1

for epoch_id in range(epoch):
	model.train()
	q_idxs = list(range(len(training_queries)))
	random.shuffle(q_idxs)

	accu_loss_total = 0.
	cdf_valid_loss = 0.
	mono_loss = 0.

	for batch_id in range(num_batches):
		batch_idxs = q_idxs[batch_id * bs: batch_id * bs + bs]
		batch_queries = extract_sublist(training_queries, batch_idxs)
		batch_sels = extract_sublist(training_sels, batch_idxs)

		batch_cdfs, batch_signs, max_num_cdfs = prepare_batch(batch_queries, num_dim, num_dim)

		batch_queries_tensor = np.stack(batch_cdfs, axis=0)
		batch_queries_tensor = torch.FloatTensor(batch_queries_tensor)

		batch_signs_tensor = np.stack(batch_signs, axis=0)
		batch_signs_tensor = torch.FloatTensor(batch_signs_tensor)

		batch_sels_tensor = np.stack(batch_sels, axis=0)
		batch_sels_tensor = torch.FloatTensor(batch_sels_tensor)

		optimizer.zero_grad()

		batch_queries_tensor = batch_queries_tensor.view(-1, num_dim)
		outputs = model(batch_queries_tensor)  # [bs*max_num_cdfs, num_dims]

		sel_predicts = torch.squeeze(outputs)
		sel_predicts = sel_predicts.view(-1, max_num_cdfs)  # [bs, max_num_cdfs]
		sel_predicts = torch.sum(sel_predicts * batch_signs_tensor, dim=-1)

		criterion = torch.nn.MSELoss()
		total_loss = criterion(sel_predicts, batch_sels_tensor)

		accu_loss_total += total_loss.item()
		total_loss.backward()
		optimizer.step()

	print("epoch: {}; loss: {}".format(epoch_id, accu_loss_total / num_batches))

	model.eval()

	test_cdfs, test_signs, test_max_num_cdfs = prepare_batch(test_queries, num_dim, num_dim)

	test_cdfs = np.stack(test_cdfs, axis=0)
	test_cdfs = torch.FloatTensor(test_cdfs)

	test_signs = np.stack(test_signs, axis=0)
	test_signs = torch.FloatTensor(test_signs)

	test_training_sels = np.stack(test_sels, axis=0)
	test_training_sels = torch.FloatTensor(test_training_sels)

	test_cdfs = test_cdfs.view(-1, num_dim)
	outputs = model(test_cdfs)  # [bs*max_num_cdfs, num_dims]

	sel_predicts = torch.squeeze(outputs)
	sel_predicts = sel_predicts.view(-1, test_max_num_cdfs)  # [bs, max_num_cdfs]
	sel_predicts = torch.sum(sel_predicts * test_signs, dim=-1)

	get_qerror(sel_predicts, test_training_sels)

	#### test OOD queries
	test_cdfs, test_signs, test_max_num_cdfs = prepare_batch(ood_test_queries, num_dim, num_dim)

	test_cdfs = np.stack(test_cdfs, axis=0)
	test_cdfs = torch.FloatTensor(test_cdfs)

	test_signs = np.stack(test_signs, axis=0)
	test_signs = torch.FloatTensor(test_signs)

	test_training_sels = np.stack(ood_test_sels, axis=0)
	test_training_sels = torch.FloatTensor(test_training_sels)

	test_cdfs = test_cdfs.view(-1, num_dim)
	outputs = model(test_cdfs)  # [bs*max_num_cdfs, num_dims]

	sel_predicts = torch.squeeze(outputs)
	sel_predicts = sel_predicts.view(-1, test_max_num_cdfs)  # [bs, max_num_cdfs]
	sel_predicts = torch.sum(sel_predicts * test_signs, dim=-1)

	get_qerror(sel_predicts, test_training_sels, 'Out-of-Distribution')
